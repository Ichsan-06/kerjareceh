...
<template>
  <VContainer>
    ...
    <div v-else-if="job">
      ...
      <VAlert
        v-if="job.my_slot && !isProvider"
        :color="resolveSlotStatusVariant(mySlotStatus)"
        variant="tonal"
        class="mb-6"
        border="start"
      >
      ...
      <VRow>
        ...
        >
            ...
            <VCardItem>
              <template #prepend>
                <VAvatar
                  size="48"
                  class="me-3"
                  :variant="!job.provider?.avatar ? 'tonal' : undefined"
                  color="primary"
                >
                  <VImg
                    v-if="job.provider?.avatar"
                    :src="`/storage/${job.provider.avatar}`"
                  />
                  <span
                    v-else
                    class="text-h6"
                  >{{ job.provider?.name?.charAt(0) }}</span>
                </VAvatar>
              </template>
              ...
              <template #append>
                <VChip
                  :color="resolveStatusVariant(job.status)"
                  label
            ...
            <VTable v-else>
              ...
              <tbody>
                ...
                >
                  ...
                  <td>
                    <VChip
                      :color="resolveSlotStatusVariant(slot.status)"
                      size="small"
                      label
                    >
                      {{ slot.status }}
                  ...
                  <td>
                    <div v-if="slot.submission">
                      ...
                      <div
                        v-if="slot.submission.submission_data?.note"
                  ...
                  <td>
                    ...
                    >
                      <VBtn
                        ...
                        icon="bx-check"
                        variant="text"
                        @click="openApprovalDialog(slot, 'approved')"
                      />
                      <VBtn
                        size="small"
                        color="error"
                        icon="bx-x"
                        variant="text"
                        @click="openApprovalDialog(slot, 'rejected')"
                      />
                    </div>
                  </td>
                </tr>
          ...
          <VCard
            v-else
            title="Siapa yang mengerjakan ini?"
            class="mb-6"
          >
            <VCardText
            ...
            >
              <VListItem
                v-for="participant in publicParticipants"
                :key="participant.id"
              >
                <template #prepend>
                  <VAvatar
                ...
                <template #append>
                  <VChip
                    :color="resolveSlotStatusVariant(participant.status)"
                    size="x-small"
                    label
                    class="text-capitalize"
                  >
              ...
              >
                ...
                >
                  <VAvatar
                    size="32"
                    color="secondary"
                    :variant="!comment.user.avatar ? 'tonal' : undefined"
                  >
                    <VImg
      ...
      <VCard title="Ajukan Banding">
        <VCardText>
          <p class="text-body-2 mb-4">
            Jelaskan mengapa menurut anda penolakan ini tidak adil.
          </p>
          <VTextarea 
