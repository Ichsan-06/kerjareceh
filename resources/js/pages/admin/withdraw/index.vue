...
<template>
    <VContainer>
        <h2 class="text-h4 mb-6">Verifikasi Penarikan Saldo</h2>
                ...
                <tbody>
                    <tr v-for="req in requests" :key="req.id">
                        <td>
                            ...
                            <div class="text-caption text-medium-emphasis">{{ req.user.email }}</div>
                        </td>
                        <td class="font-weight-bold">{{ formatCurrency(req.amount) }}</td>
                        <td>
                        ...
                        <td>
                            <VChip :color="resolveStatusColor(req.status)" size="small" label class="text-capitalize">
                                {{ req.status }}
                            </VChip>
                        </td>
                        <td>
                            <div v-if="req.status === 'pending'" class="d-flex gap-2">
                                <VBtn size="small" color="success" icon="bx-check" variant="text" @click="openApprove(req)" title="Approve" />
                                <VBtn size="small" color="error" icon="bx-x" variant="text" @click="openReject(req)" title="Reject" />
                            </div>
                     ...
                     <tr v-if="requests.length === 0">
                        <td colspan="6" class="text-center py-4 text-medium-emphasis">Tidak ada permintaan penarikan.</td>
                    </tr>
                </tbody>
            </VTable>
        </VCard>
        ...
        <VDialog v-model="approveDialog" max-width="500">
            <VCard title="Setujui Penarikan">
                ...
                <VCardActions>
                    <VSpacer />
                    <VBtn color="secondary" variant="text" @click="approveDialog = false">Batal</VBtn>
                    <VBtn color="success" :loading="processing" @click="submitApprove">Konfirmasi & Upload</VBtn>
                </VCardActions>
            </VCard>
        </VDialog>
        ...
        <VDialog v-model="rejectDialog" max-width="500">
             <VCard title="Tolak Penarikan">
                ...
                <VCardActions>
                    <VSpacer />
                    <VBtn color="secondary" variant="text" @click="rejectDialog = false">Batal</VBtn>
                    <VBtn color="error" :loading="processing" @click="submitReject">Tolak Permintaan</VBtn>
                </VCardActions>
            </VCard>
        </VDialog>
    </VContainer>
