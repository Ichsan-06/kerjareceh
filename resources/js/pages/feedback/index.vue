...
<template>
  <VContainer>
    <div class="d-flex justify-space-between align-center mb-6">
       <h2 class="text-h4">Feedback</h2>
       <VBtn v-if="can('create feedback')" color="primary" @click="dialog = true">
         Send Feedback
       </VBtn>
    </div>
    ...
    <VCard v-if="can('read feedback')">
        ...
        >
            <template #item.type="{ item }">
                <VChip :color="resolveTypeColor(item.type)" size="small" label class="text-capitalize">
                    {{ item.type }}
                </VChip>
            </template>

            <template #item.status="{ item }">
                <VChip color="default" size="small" label class="text-capitalize">
                    {{ item.status }}
                </VChip>
            </template>
    ...
    <VDialog v-model="dialog" max-width="500">
        <VCard title="Send Feedback">
            <VCardText>
                <VForm v-model="valid" @submit.prevent="submitFeedback">
                    <VSelect
                        v-model="form.type"
            ...
            <VCardActions>
                ...
                <VBtn color="primary" @click="submitFeedback">Send</VBtn>
            </VCardActions>
        </VCard>
    </VDialog>
  </VContainer>
